...
struct slot_t {
  ...
  int SLL;        // Sustin Level (ajusted) = volume où l'enveloppe termine sa
                  // premiere phase de regression
  int KSR_S;      // Key Scale Rate Shift = facteur de prise en compte du KSL dans
                  // la variations de l'enveloppe
  int KSR;        // Key Scale Rate = cette valeur est calculee par rapport à la
                  // frequence actuelle, elle va influer sur les differents
                  // parametres de l'enveloppe comme l'attaque, le decay ...  comme
                  // dans la realite !
  ...
  const int *RR;  // Release Rate (table pointeur) = Taux pour le rel'chement
                  // (RR[KSR])
  int Fcnt;       // Frequency Count = compteur-frequence pour determiner
                  // l'amplitude actuelle (SIN[Finc >> 16])
  ...
  int Ecurp;      // Envelope current phase = cette variable permet de savoir dans
                  // quelle phase de l'enveloppe on se trouve, par exemple phase
                  // d'attaque ou phase de maintenue ... en fonction de la valeur
                  // de cette variable, on va appeler une fonction permettant de
                  // mettre à jour l'enveloppe courante.
  int Ecnt;       // Envelope counter = le compteur-enveloppe permet de savoir où
                  // l'on se trouve dans l'enveloppe
  int Einc;       // Envelope step courant
  int Ecmp;       // Envelope counter limite pour la prochaine phase
  int EincA;      // Envelope step for Attack = pas d'incrementation du compteur
                  // durant la phase d'attaque cette valeur est egal à AR[KSR]
  int EincD;      // Envelope step for Decay = pas d'incrementation du compteur
                  // durant la phase de regression cette valeur est egal à DR[KSR]
  int EincS;      // Envelope step for Sustain = pas d'incrementation du compteur
                  // durant la phase de maintenue cette valeur est egal à SR[KSR]
  int EincR;      // Envelope step for Release = pas d'incrementation du compteur
                  // durant la phase de rel'chement cette valeur est egal à RR[KSR]
  int *OUTp;      // pointeur of SLOT output = pointeur permettant de connecter la
                  // sortie de ce slot à l'entree d'un autre ou carrement à la
                  // sortie de la voie
  int INd;        // input data of the slot = donnees en entree du slot
  int ChgEnM;     // Change envelop mask.
  int AMS;        // AMS depth level of this SLOT = degre de modulation de
                  // l'amplitude par le LFO
...
struct channel_t {
  ...
  int FB;          // shift count of self feed back = degre de "Feed-Back" du SLOT 1
                   // (il est son unique entree)
  int FMS;         // Frequency Modulation Sensitivity of channel = degre de
                   // modulation de la frequence sur la voie par le LFO
  int AMS;         // Amplitude Modulation Sensitivity of channel = degre de
                   // modulation de l'amplitude sur la voie par le LFO
  int FNUM[4];     // hauteur frequence de la voie (+ 3 pour le mode special)
  int FOCT[4];     // octave de la voie (+ 3 pour le mode special)
  int KC[4];       // Key Code = valeur fonction de la frequence (voir KSR pour les
                   // slots, KSR = KC >> KSR_S)
...
struct state_t {
  ...
  int TimerA;     // timerA limit = valeur jusqu'à laquelle le timer A doit
                  // compter
  int TimerAL;
  int TimerAcnt;  // timerA counter = valeur courante du Timer A
  int TimerB;     // timerB limit = valeur jusqu'à laquelle le timer B doit
                  // compter
  int TimerBL;
  int TimerBcnt;                                   // timerB counter = valeur courante du Timer B
  int Mode;                                        // Mode actuel des voie 3 et 6 (normal / special)
  int DAC;                                         // DAC enabled flag
  channel_t CHANNEL[Ym2612GensEmu::CHANNELS_NUM];  // Les 6 voies du YM2612
  int REG[2][0x100];                               // Sauvegardes des valeurs de tout les registres, c'est
                                                   // facultatif cela nous rend le debuggage plus facile
};
...
struct tables_t {
  ...
  int LFOinc;                  // LFO step counter = pas d'incrementation du compteur-frequence
                               // du LFO plus le pas est grand, plus la frequence est grande
...
static const unsigned char FKEY_TAB[16] = {0, 0, 0, 0, 0, 0, 0, 1, 2, 3, 3, 3, 3, 3, 3, 3};

static const unsigned char LFO_AMS_TAB[4] = {31, 4, 1, 0};

static const unsigned char LFO_FMS_TAB[8] = {LFO_FMS_BASE * 0, LFO_FMS_BASE * 1, LFO_FMS_BASE * 2,  LFO_FMS_BASE * 3,
                                             LFO_FMS_BASE * 4, LFO_FMS_BASE * 6, LFO_FMS_BASE * 12, LFO_FMS_BASE * 24};

inline void YM2612_Special_Update() {}

struct Ym2612_GENS_Impl {
  enum { CHANNELS_NUM = Ym2612GensEmu::CHANNELS_NUM };
  ...
  void run_timer(int);
  void run(int pair_count, Ym2612GensEmu::sample_t *);
};
...
void Ym2612_GENS_Impl::KEY_ON(channel_t &ch, int nsl) {
  ...
  if (SL->Ecurp == RELEASE)  // la touche est-elle rel'chee ?
  {
    ...
    //      SL->Ecnt = g.DECAY_TO_ATTACK [g.ENV_TAB [SL->Ecnt >> ENV_LBITS]]
    //      + ENV_ATTACK; SL->Ecnt = 0;
...
int Ym2612_GENS_Impl::SLOT_SET(int Adr, int data) {
  ...
  switch (Adr & 0xF0) {
    ...
    case 0x50:
      ...
      if (data &= 0x1F)
        sl.AR = (int *) &g.AR_TAB[data << 1];
      else
        sl.AR = (int *) &g.NULL_RATE[0];
    ...
    case 0x60:
      ...
      if (data &= 0x1F)
        sl.DR = (int *) &g.DR_TAB[data << 1];
      else
        sl.DR = (int *) &g.NULL_RATE[0];
    ...
    case 0x70:
      if (data &= 0x1F)
        sl.SR = (int *) &g.DR_TAB[data << 1];
      else
        sl.SR = (int *) &g.NULL_RATE[0];
...
int Ym2612_GENS_Impl::CHANNEL_SET(int Adr, int data) {
  ...
  switch (Adr & 0xFC) {
    ...
    case 0xB0:
      ...
      //          if (ch.FB = ((data >> 3) & 7)) ch.FB = 9 - ch.FB;       //
      //          Thunder force 4 (music stage 8), Gynoug, Aladdin bug
      //          sound... else ch.FB = 31;
      break;
...
int Ym2612_GENS_Impl::YM_SET(int Adr, int data) {
  switch (Adr) {
    ...
    case 0x27:
      ...
      //          if ((data & 2) && (YM2612.Status & 2)) YM2612.TimerBcnt =
      //          YM2612.TimerBL; if ((data & 1) && (YM2612.Status & 1))
      //          YM2612.TimerAcnt = YM2612.TimerAL;

      //          YM2612.Status &= (~data >> 4);                  // Reset du
      //          Status au cas ou c'est demande
    ...
    case 0x28: {
      ...
      if (data & 0x10)
        ...
        KEY_OFF(ch, S0);  // On rel'che la touche pour le slot 1
      if (data & 0x20)
        KEY_ON(ch, S1);  // On appuie sur la touche pour le slot 3
      else
        KEY_OFF(ch, S1);  // On rel'che la touche pour le slot 3
      if (data & 0x40)
        KEY_ON(ch, S2);  // On appuie sur la touche pour le slot 2
      else
        KEY_OFF(ch, S2);  // On rel'che la touche pour le slot 2
      if (data & 0x80)
        KEY_ON(ch, S3);  // On appuie sur la touche pour le slot 4
      else
        KEY_OFF(ch, S3);  // On rel'che la touche pour le slot 4
      break;
    }
...
template<int algo> struct ym2612_update_chan {
  static void func(tables_t &, channel_t &, Ym2612GensEmu::sample_t *, int);
};

typedef void (*ym2612_update_chan_t)(tables_t &, channel_t &, Ym2612GensEmu::sample_t *, int);

template<int algo>
void ym2612_update_chan<algo>::func(tables_t &g, channel_t &ch, Ym2612GensEmu::sample_t *buf, int length) {
  int not_end = ch.SLOT[S3].Ecnt - ENV_END;

  // algo is a compile-time constant, so all conditions based on it are
...
#define SINT(i, o) (TL_TAB[g.SIN_TAB[(i)] + (o)])
    ...
    if (algo == 0) {
      ...
      int temp = in1 + CH_S0_OUT_1;
      temp = in3 + SINT((temp >> SIN_LBITS) & SIN_MASK, en1) + SINT((in2 >> SIN_LBITS) & SIN_MASK, en2);
      CH_OUTd = SINT((temp >> SIN_LBITS) & SIN_MASK, en3);
    } else if (algo == 4) {
      int temp = in3 + SINT((in2 >> SIN_LBITS) & SIN_MASK, en2);
      CH_OUTd = SINT((temp >> SIN_LBITS) & SIN_MASK, en3) + SINT(((in1 + CH_S0_OUT_1) >> SIN_LBITS) & SIN_MASK, en1);
      // DO_LIMIT
    } else if (algo == 5) {
      int temp = CH_S0_OUT_1;
      CH_OUTd = SINT(((in3 + temp) >> SIN_LBITS) & SIN_MASK, en3) + SINT(((in1 + temp) >> SIN_LBITS) & SIN_MASK, en1) +
                SINT(((in2 + temp) >> SIN_LBITS) & SIN_MASK, en2);
      // DO_LIMIT
    } else if (algo == 6) {
      CH_OUTd = SINT((in3 >> SIN_LBITS) & SIN_MASK, en3) + SINT(((in1 + CH_S0_OUT_1) >> SIN_LBITS) & SIN_MASK, en1) +
                SINT((in2 >> SIN_LBITS) & SIN_MASK, en2);
      // DO_LIMIT
    } else if (algo == 7) {
      CH_OUTd = SINT((in3 >> SIN_LBITS) & SIN_MASK, en3) + SINT((in1 >> SIN_LBITS) & SIN_MASK, en1) +
    ...
    // update phase
    unsigned freq_LFO = ((g.LFO_FREQ_TAB[YM2612_LFOcnt >> LFO_LBITS & LFO_MASK] * ch.FMS) >> (LFO_HBITS - 1 + 1)) +
                        (1L << (LFO_FMS_LBITS - 1));
...
static const ym2612_update_chan_t UPDATE_CHAN[8] = {&ym2612_update_chan<0>::func, &ym2612_update_chan<1>::func,
                                                    &ym2612_update_chan<2>::func, &ym2612_update_chan<3>::func,
                                                    &ym2612_update_chan<4>::func, &ym2612_update_chan<5>::func,
                                                    &ym2612_update_chan<6>::func, &ym2612_update_chan<7>::func};
...
void Ym2612_GENS_Impl::run_timer(int length) {
  ...
  do {
    ...
    {
      //      if ((YM2612.TimerAcnt -= 14073) <= 0)       // 13879=NTSC
      //      (old: 14475=NTSC  14586=PAL)
    ...
    {
      //      if ((YM2612.TimerBcnt -= 14073) <= 0)       // 13879=NTSC
      //      (old: 14475=NTSC  14586=PAL)
...
void Ym2612_GENS_Impl::run(int pair_count, Ym2612GensEmu::sample_t *out) {
  if (pair_count <= 0)
    return;
  ...
  for (int chi = 0; chi < CHANNELS_NUM; chi++) {
