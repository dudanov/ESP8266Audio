...
static uint8_t const initial_regs[SpcDsp::REGISTERS_NUM] = {
    ...
    0x00, 0x9A, 0xC5, 0x9C, 0x4E, 0x7B, 0xFF, 0xEA, 0x21, 0x78, 0x4F, 0xDD, 0xED, 0x24, 0x14, 0x00, 0x00, 0x77, 0xB1,
    0xD1, 0x36, 0xC1, 0x67, 0x52, 0x57, 0x46, 0x3D, 0x59, 0xF4, 0x87, 0xA4, 0x00, 0x00, 0x7E, 0x44, 0x9C, 0x4E, 0x7B,
    0xFF, 0x75, 0xF5, 0x06, 0x97, 0x10, 0xC3, 0x24, 0xBB, 0x00, 0x00, 0x7B, 0x7A, 0xE0, 0x60, 0x12, 0x0F, 0xF7, 0x74,
    0x1C, 0xE5, 0x39, 0x3D, 0x73, 0xC1, 0x00, 0x00, 0x7A, 0xB3, 0xFF, 0x4E, 0x7B, 0xFF};
...
#define WRITE_SAMPLES(l, r, out) \
  { \
    ...
    if (out >= m.out_end) { \
      check(out == m.out_end); \
      check(m.out_end != &m.extra[EXTRA_SIZE] || (m.extra <= m.out_begin && m.extra < &m.extra[EXTRA_SIZE])); \
      out = m.extra; \
      m.out_end = &m.extra[EXTRA_SIZE]; \
    } \
  }

void SpcDsp::set_output(sample_t *out, int size) {
...
static short const interleved_gauss[512] = {
    ...
    263, 1283, 260, 1282, 257, 1280, 254, 1279, 251, 1277, 248, 1275, 245, 1274, 242, 1272, 239, 1270, 236, 1269,
    233, 1267, 230, 1265, 227, 1263, 224, 1261, 221, 1259, 218, 1257, 215, 1255, 212, 1253, 210, 1251, 207, 1248,
    204, 1246, 201, 1244, 199, 1241, 196, 1239, 193, 1237, 191, 1234, 188, 1232, 186, 1229, 183, 1227, 180, 1224,
    178, 1221, 175, 1219, 173, 1216, 171, 1213, 168, 1210, 166, 1207, 163, 1205, 161, 1202, 159, 1199, 156, 1196,
    154, 1193, 152, 1190, 150, 1186, 147, 1183, 145, 1180, 143, 1177, 141, 1174, 139, 1170, 137, 1167, 134, 1164,
    132, 1160, 130, 1157, 128, 1153, 126, 1150, 124, 1146, 122, 1143, 120, 1139, 118, 1136, 117, 1132, 115, 1128,
    113, 1125, 111, 1121, 109, 1117, 107, 1113, 106, 1109, 104, 1106, 102, 1102, 100, 1098, 99,  1094, 97,  1090,
    95,  1086, 94,  1082, 92,  1078, 90,  1074, 89,  1070, 87,  1066, 86,  1061, 84,  1057, 83,  1053, 81,  1049,
    80,  1045, 78,  1040, 77,  1036, 76,  1032, 74,  1027, 73,  1023, 71,  1019, 70,  1014, 69,  1010, 67,  1005,
    66,  1001, 65,  997,  64,  992,  62,  988,  61,  983,  60,  978,  59,  974,  58,  969,  56,  965,  55,  960,
    54,  955,  53,  951,  52,  946,  51,  941,  50,  937,  49,  932,  48,  927,  47,  923,  46,  918,  45,  913,
    44,  908,  43,  904,  42,  899,  41,  894,  40,  889,  39,  884,  38,  880,  37,  875,  36,  870,  36,  865,
    35,  860,  34,  855,  33,  851,  32,  846,  32,  841,  31,  836,  30,  831,  29,  826,  29,  821,  28,  816,
    27,  811,  27,  806,  26,  802,  25,  797,  24,  792,  24,  787,  23,  782,  23,  777,  22,  772,  21,  767,
    21,  762,  20,  757,  20,  752,  19,  747,  19,  742,  18,  737,  17,  732,  17,  728,  16,  723,  16,  718,
    15,  713,  15,  708,  15,  703,  14,  698,  14,  693,  13,  688,  13,  683,  12,  678,  12,  674,  11,  669,
    11,  664,  11,  659,  10,  654,  10,  649,  10,  644,  9,   640,  9,   635,  9,   630,  8,   625,  8,   620,
    8,   615,  7,   611,  7,   606,  7,   601,  6,   596,  6,   592,  6,   587,  6,   582,  5,   577,  5,   573,
    5,   568,  5,   563,  4,   559,  4,   554,  4,   550,  4,   545,  4,   540,  3,   536,  3,   531,  3,   527,
    3,   522,  3,   517,  2,   513,  2,   508,  2,   504,  2,   499,  2,   495,  2,   491,  2,   486,  1,   482,
    1,   477,  1,   473,  1,   469,  1,   464,  1,   460,  1,   456,  1,   451,  1,   447,  1,   443,  1,   439,
    0,   434,  0,   430,  0,   426,  0,   422,  0,   418,  0,   414,  0,   410,  0,   405,  0,   401,  0,   397,
...
static unsigned const counter_mask[32] = {
    ...
    RATE(10, 5),  RATE(8, 4),    RATE(6, 3),    RATE(5, 5),    RATE(4, 4),    RATE(3, 3),   RATE(2, 4),   RATE(1, 4)};
#undef RATE

inline void SpcDsp::init_counter() {
...
inline void SpcDsp::run_counter(int i) {
  int n = m.counters[i];
  if (!(n-- & 7))
    n -= 6 - i;
  m.counters[i] = n;
...
#define READ_COUNTER(rate) (*m.counter_select[rate] & counter_mask[rate])

//// Emulation

void SpcDsp::run(int clock_count) {
...
#define SAMPLE_PTR(i) GET_LE16A(&dir[VREG(v_regs, SRCN) * 4 + i * 2])
      ...
      // Gaussian interpolation
      {
        ...
        if (env) {
          ...
          {
            // Faster approximation when exact sample value isn't
            // necessary for pitch mod
      ...
      if (!v->kon_delay) {
        ...
        } else  // 3%
        {
          ...
          // unsigned cast because linear decrease going negative also
          // triggers this
...

    {
      ...
      if (old_pos >= 0x4000) {
        // Arrange the four input nybbles in 0xABCD order for easy
        // decoding
        int nybbles =
            ram[(v->brr_addr + v->brr_offset) & 0xFFFF] * 0x100 + ram[(v->brr_addr + v->brr_offset + 1) & 0xFFFF];
        ...
        static unsigned char const shifts[16 * 2] = {13, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 16, 16, 16,
        ...
        for (end = pos + 4; pos < end; pos++, nybbles <<= 4) {
          // Extract upper nybble and scale appropriately. Every cast
          // is necessary to maintain correctness and avoid undef
          // behavior
...
#ifdef SPC_ISOLATED_ECHO_BUFFER
    // And here, we win no awards for accuracy, but gain playback of dodgy
    // Super Mario World mod SPCs
...
void SpcDsp::mute_voices(int mask) {
  m.mute_mask = mask;
