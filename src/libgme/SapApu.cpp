...
#define POLY_MASK(width, tap1, tap2) ((1UL << (width - 1 - tap1)) | (1UL << (width - 1 - tap2)))

SapApuImpl::SapApuImpl() {
...
void SapApu::reset(SapApuImpl *new_impl) {
  impl = new_impl;
  last_time = 0;
  poly5_pos = 0;
  poly4_pos = 0;
...
inline void SapApu::calc_periods() {
  // 15/64 kHz clock
  int divider = 28;
  if (this->control & 1)
    divider = 114;
  ...
  for (int i = 0; i < OSCS_NUM; i++) {
    ...
    if (output) {
      ...
      if (!volume || osc_control & 0x10 ||  // silent, DAC mode, or inaudible frequency
        ...
        // high pass
        static uint8_t const hipass_bits[OSCS_NUM] = {1 << 2, 1 << 1, 0, 0};
        ...
        if (time < end_time || time2 < end_time) {
          ...
          // Run wave and high pass interleved with each catching up
          // to the other. Disabled high pass has no performance
          // effect since inner wave loop makes no compromise for high
          // pass, and only runs once in that case.
