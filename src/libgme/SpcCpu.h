...
#define CPU_READ(time, offset, addr) cpu_read(addr, time + offset)

#define CPU_WRITE(time, offset, addr, data) cpu_write(data, addr, time + offset)
...
unsigned SnesSpc::CPU_mem_bit(uint16_t pc, rel_time_t rel_time) {
...
loop : {
  ...
  */
  ...
  // TODO: if PC is at end of memory, this will get wrong operand (very
  // obscure)
  pc++;
  data = ram[pc];
  switch (opcode) {
  ...
  case op:              /* imm */ \
    ...
    { \
      ...
      case op + 0x10: { /*dp,imm*/ \
        ...
        addr_##op : nz = data func READ(-1, addr); \
        WRITE(0, addr, nz); \
        goto loop; \
    }
