...
static const int RAM_ADDR = 0x2000;
...
void HesCpu::set_mmr(int reg, int bank) {
...
bool HesCpu::run(hes_time_t end_time) {
...
#ifndef NDEBUG
{
  ...
  if (!(status & ST_I) && correct > m_irqTime)
    correct = m_irqTime;
...
#ifdef HES_CPU_LOG_H
  log_cpu("new", pc - 1, opcode, instr[0], instr[1], instr[2], instr[3], instr[4], instr[5]);
  // log_opcode( opcode );
#endif
...
#define BRANCH(cond) \
  { \
    ...
    bit_common:
      ...
      status |= nz & ST_V;
      if (nz & a)
        goto loop;  // Z should be clear, and nz must be non-zero if nz & a
                    // is
    ...
    tst_common:
      ...
      status |= nz & ST_V;
      if (nz & data)
        goto loop;  // Z should be clear, and nz must be non-zero if nz &
                    // data is
      nz <<= 8;     // set Z flag without affecting N flag
      goto loop;
  ...
  { \
    ...
    case 0x40: {  // RTI
      ...
      if ((data ^ status) & ST_I) {
        ...
        if (!(status & ST_I) && new_time > m_irqTime)
          new_time = m_irqTime;
...
#undef POP
    ...
    handle_cli : {
      this->r.status = status;  // update externally-visible I flag
      blargg_long delta = s.base - m_irqTime;
      if (delta <= 0) {
        if (TIME < m_irqTime)
          goto loop;
        goto delayed_cli;
      ...
      if (delta >= s_time + 1) {
        ...
        s.base += s_time + 1;
        s_time = -1;
        m_irqTime = s.base;  // TODO: remove, as only to satisfy debug check in loop
        goto loop;
      }
    delayed_cli:
    ...
    case 0xF4: {  // SET
...

      {
        ...
        bxfer:
          ...
          FLUSH_TIME();
          do {
            // TODO: reads from $0800-$1400 in I/O page return 0 and don't
            // access I/O
