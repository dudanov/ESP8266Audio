...
inline void GbCpu::m_setCodePage(int i, uint8_t *p) {
  m_pState->code_map[i] = p - PAGE_OFFSET(i * (blargg_long) PAGE_SIZE);
}

void GbCpu::reset(void *unmapped) {
  check(m_pState == &m_state);
  m_pState = &m_state;

  m_state.remain = 0;

  for (int i = 0; i < PAGE_COUNT + 1; i++)
    m_setCodePage(i, (uint8_t *) unmapped);
...
void GbCpu::map_code(gb_addr_t start, unsigned size, void *data) {
  ...
  for (unsigned i = size / PAGE_SIZE; i--;)
    m_setCodePage(first_page + i, (uint8_t *) data + i * PAGE_SIZE);
}
...
bool GbCpu::run(blargg_long cycle_count) {
  m_state.remain = blargg_ulong(cycle_count + CLOCKS_PER_INSTRUCTION) / CLOCKS_PER_INSTRUCTION;
  state_t s;
  this->m_pState = &s;
  memcpy(&s, &this->m_state, sizeof s);
  ...
  { \
    ...
    case 0xCB:
      ...
      switch (data) {
        ...
        case 0x3E:                         // SRL (HL)
          data += 0x10; /* fallthrough */  // bump up to 0x4n to avoid
                                           // preserving sign bit
        case 0x1E:                         // RR (HL)
        case 0x0E:                         // RRC (HL)
        case 0x2E:                         // SRA (HL)
...
stop:
  ...
  this->m_pState = &m_state;
  memcpy(&this->m_state, &s, sizeof this->m_state);
