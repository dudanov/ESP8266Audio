...
void HesApu::reset() {
  ...
  do {
    osc--;
    memset(osc, 0, offsetof(HesOsc, outputs));
    osc->noise_lfsr = 1;
    osc->control = 0x40;
    osc->balance = 0xFF;
  } while (osc != oscs);
...
void HesApu::osc_output(int index, BlipBuffer *center, BlipBuffer *left, BlipBuffer *right) {
...
void HesOsc::mRunUntil(synth_t &synth_, blip_time_t end_time) {
  ...
  if (osc_outputs_0 && control & 0x80) {
    ...
    int const volume_0 = volume[0];
    {
      ...
      if (delta)
        synth_.Offset(osc_outputs_0, last_time, delta);
    ...
    if (osc_outputs_1) {
      ...
      if (delta)
        synth_.Offset(osc_outputs_1, last_time, delta);
      ...
      if (noise & 0x80) {
        if (volume_0 | volume_1) {
          ...
          do {
            ...
            // noise_lfsr = (noise_lfsr >> 1) ^ (0x6000 &
            // -(noise_lfsr & 1));
            int delta = new_dac - dac;
            if (delta) {
              dac = new_dac;
              synth_.Offset(osc_outputs_0, time, delta * volume_0);
              if (osc_outputs_1)
                synth_.Offset(osc_outputs_1, time, delta * volume_1);
            }
            time += period;
          } while (time < end_time);
        ...
        if (period >= 14 && (volume_0 | volume_1)) {
          ...
          if (!period) {
            // TODO: Gekisha Boy assumes that period = 0 silences
            // wave
            // period = 0x1000 * 2;
            period = 1;
            // if ( !(volume_0 | volume_1) )
...
void HesApu::balance_changed(HesOsc &osc) {
  static short const log_table[32] = {
  // ~1.5 db per step
#define ENTRY(factor) short(factor * HesOsc::amp_range / 31.0 + 0.5)
      ENTRY(0.000000), ENTRY(0.005524), ENTRY(0.006570), ENTRY(0.007813), ENTRY(0.009291), ENTRY(0.011049),
      ENTRY(0.013139), ENTRY(0.015625), ENTRY(0.018581), ENTRY(0.022097), ENTRY(0.026278), ENTRY(0.031250),
      ENTRY(0.037163), ENTRY(0.044194), ENTRY(0.052556), ENTRY(0.062500), ENTRY(0.074325), ENTRY(0.088388),
      ENTRY(0.105112), ENTRY(0.125000), ENTRY(0.148651), ENTRY(0.176777), ENTRY(0.210224), ENTRY(0.250000),
      ENTRY(0.297302), ENTRY(0.353553), ENTRY(0.420448), ENTRY(0.500000), ENTRY(0.594604), ENTRY(0.707107),
      ENTRY(0.840896), ENTRY(1.000000),
#undef ENTRY
  };
...
void HesApu::write_data(blip_time_t time, int addr, int data) {
  if (addr == 0x800) {
    latch = data & 7;
  } else if (addr == 0x801) {
    if (balance != data) {
...
void HesApu::end_frame(blip_time_t end_time) {
  HesOsc *osc = &oscs[OSCS_NUM];
  do {
    osc--;
