...
static char *parse_filename(char *in, M3uPlaylist::entry_t &entry) {
  entry.file = in;
  entry.type = "";
  char *out = in;
  while (1) {
...
static char *parse_int(char *in, int *out, int *result) { return next_field(parse_int_(in, out), result); }
...
static char *parse_track(char *in, M3uPlaylist::entry_t &entry, int *result) {
  if (*in == '$') {
    in++;
    int n = 0;
    while (1) {
...
static char *parse_time(char *in, int *out, int *result) { return next_field(parse_time_(in, out), result); }
...
static int parse_line(char *in, M3uPlaylist::entry_t &entry) {
  int result = 0;
...
static void parse_comment(char *in, M3uPlaylist::info_t &info, bool first) {
  in = skip_white(in + 1);
  const char *field = in;
  while (*in && *in != ':')
    in++;
...
blargg_err_t M3uPlaylist::parse_() {
  info_.title = "";
  info_.composer = "";
  info_.engineer = "";
  info_.ripping = "";
...
blargg_err_t M3uPlaylist::parse() {
...
blargg_err_t M3uPlaylist::load(DataReader &in) {
