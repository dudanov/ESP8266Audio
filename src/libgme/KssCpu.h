...
void kss_cpu_out(class KssCpu *, cpu_time_t, unsigned addr, int data);
int kss_cpu_in(class KssCpu *, cpu_time_t, unsigned addr);
void kss_cpu_write(class KssCpu *, unsigned addr, int data);
...
inline uint8_t *KssCpu::write(unsigned addr) { return state->write[addr >> page_shift] + KSS_CPU_PAGE_OFFSET(addr); }

inline uint8_t const *KssCpu::read(unsigned addr) {
  return state->read[addr >> page_shift] + KSS_CPU_PAGE_OFFSET(addr);
}

inline void KssCpu::set_end_time(cpu_time_t t) {
