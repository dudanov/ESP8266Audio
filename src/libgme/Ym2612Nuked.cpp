...
/* logsin table */
static const Bit16u LOG_SIN_TABLE[] = {
...
/* Note table */
static const Bit32u fn_note[16] = {0, 0, 0, 0, 0, 0, 0, 1, 2, 3, 3, 3, 3, 3, 3, 3};

/* Envelope generator */
static const Bit32u eg_stephi[4][4] = {{0, 0, 0, 0}, {1, 0, 0, 0}, {1, 0, 1, 0}, {1, 1, 1, 0}};

static const Bit8u eg_am_shift[4] = {7, 3, 1, 0};

/* Phase generator */
static const Bit32u pg_detune[8] = {16, 17, 19, 20, 22, 24, 27, 29};

static const Bit32u pg_lfo_sh1[8][8] = {{7, 7, 7, 7, 7, 7, 7, 7}, {7, 7, 7, 7, 7, 7, 7, 7}, {7, 7, 7, 7, 7, 7, 1, 1},
                                        {7, 7, 7, 7, 1, 1, 1, 1}, {7, 7, 7, 1, 1, 1, 1, 0}, {7, 7, 1, 1, 0, 0, 0, 0},
                                        {7, 7, 1, 1, 0, 0, 0, 0}, {7, 7, 1, 1, 0, 0, 0, 0}};

static const Bit32u pg_lfo_sh2[8][8] = {{7, 7, 7, 7, 7, 7, 7, 7}, {7, 7, 7, 7, 2, 2, 2, 2}, {7, 7, 7, 2, 2, 2, 7, 7},
                                        {7, 7, 2, 2, 7, 7, 2, 2}, {7, 7, 2, 7, 7, 7, 2, 7}, {7, 7, 7, 2, 7, 7, 2, 1},
...
/* LFO */
static const Bit32u lfo_cycles[8] = {108, 77, 71, 67, 62, 44, 8, 5};
...
void OPN2_EnvelopeSSGEG(ym3438_t *chip) {
  ...
  if (chip->ssg_eg[slot] & 0x08) {
    ...
    chip->eg_ssg_inv[slot] = (chip->eg_ssg_dir[slot] ^ ((chip->ssg_eg[slot] >> 2) & 0x01)) & chip->eg_kon[slot];
  }
...
void OPN2_FMGenerate(ym3438_t *chip) {
  ...
  output = ((EXP_TABLE[(level & 0xff) ^ 0xff] | 0x400) << 2) >> (level >> 8);
  if (phase & 0x200) {
...
void OPN2_SetChipType(Bit32u type) { chip_type = type; }
...
void OPN2_SetTestPin(ym3438_t *chip, Bit32u value) { chip->pin_test_in = value & 1; }
...
Bit32u OPN2_ReadIRQPin(ym3438_t *chip) { return chip->timer_a_overflow_flag | chip->timer_b_overflow_flag; }
...
Bit8u OPN2_Read(ym3438_t *chip, Bit32u port) {
  if ((port & 3) == 0 || chip_type == ym3438_type_asic) {
    if (chip->mode_test_21[6]) {
      /* Read test data */
      Bit32u slot = (chip->cycles + 18) % 24;
      Bit16u testdata = ((chip->pg_read & 0x01) << 15) | ((chip->eg_read[chip->mode_test_21[0]] & 0x01) << 14);
      ...
      return (Bit8u)(chip->busy << 7) | (Bit8u)(chip->timer_b_overflow_flag << 1) | (Bit8u) chip->timer_a_overflow_flag;
    }
  }
  return 0;
}
...
void OPN2_WriteBuffered(ym3438_t *chip, Bit32u port, Bit8u data) {
  ...
  if (chip->writebuf[chip->writebuf_last].port & 0x04) {
    OPN2_Write(chip, chip->writebuf[chip->writebuf_last].port & 0X03, chip->writebuf[chip->writebuf_last].data);
...
void OPN2_GenerateResampled(ym3438_t *chip, Bit16s *buf) {
  ...
  buf[0] = (Bit16s)(((chip->oldsamples[0] * (chip->rateratio - chip->samplecnt) + chip->samples[0] * chip->samplecnt) /
                     chip->rateratio) >>
                    1);
  buf[1] = (Bit16s)(((chip->oldsamples[1] * (chip->rateratio - chip->samplecnt) + chip->samples[1] * chip->samplecnt) /
                     chip->rateratio) >>
                    1);
