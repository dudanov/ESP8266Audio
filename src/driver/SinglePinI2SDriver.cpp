...
bool SinglePinI2SDriver::begin(uint8_t bufCount, uint16_t bufSize) {
  if (bufCount < 4)
    return false;
  if ((bufSize < 32) || (bufSize > 1020) || ((bufSize % 4) != 0))
    return false;

  stopSLC();

  if ((bufCount != this->bufCount) || (bufSize != this->bufSize))
    freeBuffers(this->bufCount);
...
void SinglePinI2SDriver::stop() { stopI2S(); }
...
bool SinglePinI2SDriver::write(uint32_t sample) {
  ...
  if (isSLCRunning() && (lastSent->next_link_ptr == head))
    return false;
