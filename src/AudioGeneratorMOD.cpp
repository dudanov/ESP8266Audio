...
void AudioGeneratorMOD::GetSample(int16_t sample[2]) {
  ...
  for (channel = 0; channel < Mod.numberOfChannels; channel++) {
    ...
    samplePointer =
        Mixer.sampleBegin[Mixer.channelSampleNumber[channel]] + (Mixer.channelSampleOffset[channel] >> FIXED_DIVIDER);
    ...
    next =
        FatBuffer.channels[channel][(samplePointer + 1 - FatBuffer.samplePointer[channel]) /*& (FATBUFFERSIZE - 1)*/];
    ...
    // Actually we could could even gain more bits, I was just not sure if more bits would cause overflows in other
    // conputations.
  ...
  // Downscale to BITDEPTH - a bit faster because the compiler can replaced division by constants with proper "right
  // shift" + correct handling of sign bit
